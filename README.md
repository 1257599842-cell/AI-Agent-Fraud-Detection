...
```bash
uvicorn src.serving.app:app --port 8000     # /score  /investigate  /healthz
...
结果 test ROC **0.872 → 0.848**（每轮约 1,774 笔真欺诈被误标成好人）；
加 **2% 随机放行抽检**后回到 0.866，**救回 +0.018**。
...
| 选择性偏差演示 | ROC 0.872 → 0.848 → 抽检**救回 +0.018** |
...
4. **velocity 规则未实现**：能接住「低分 + 微额」盲区的跨笔规则，已论证、未开工。
