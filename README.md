...
> 把金额往上拖，最优动作会在两个临界点依次翻转：**$74.33 → 挂起**、**$405.00 → 拒绝**。
> 同一条曲线上还有第三个临界点 **$2.44**——低于它连 $0.50 的验证摩擦都不划算，最优动作退回放行。
> **这一点同时暴露了逐笔决策口径的固有边界**：试卡（card testing）正是 <$1 的微额欺诈，逐笔算永远划不来拦，
> 它的价值不在本笔而在为后续大额盗刷验证卡的活性。**能接住它的是跨笔的 velocity 规则，而那是本项目已论证、未实现的缺口**（见 `MODEL_CARD.md` §8）。
