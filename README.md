...
> 2026 秋招旗舰项目。完整设计与工作守则见 [`CLAUDE.md`](./CLAUDE.md)，实时进度见 [`PROGRESS.md`](./PROGRESS.md)。面试答案库（`INTERVIEW.md`）与背诵卡未随仓库发布。
...
另有一份《作废清单》记录测过之后决定**不用**的十一件事（随答案库留在本地）。
...
> 每条防守点的 30 秒 / 2 分钟答案另存于本地答案库，不随仓库发布。
