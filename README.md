...
> 单人项目 · 2026。设计口径、假设与已知局限见下方 `MODEL_CARD.md`；开发期工作日志未随仓库发布。
> **交付文档见 [`MODEL_CARD.md`](./MODEL_CARD.md)** —— 模型卡 / 模型验证报告，含用途边界、口径、假设、**已知局限与偏差**、监控与退化触发、降级方案、以及"已推演但未实现"的工业化路径。

这份 README 是**滚动复盘**，边做边写。
**当前状态**：四层全部落地（ML 核心 / 图特征 / Agent + eval / FastAPI·Docker），十个硬点全部有真实数字。
另有一节记录**测过之后决定不用**的十五件事——见 [`MODEL_CARD.md`](./MODEL_CARD.md) §12.5「已作废的结论」。
...
> 把金额往上拖，最优动作依次翻转（页面默认 p=0.302 上）：**$73.88 → 挂起**、**$400.71 → 拒绝**。
> 临界点随 p 变化，闭式解为 `a* = c_friction / [p·r_block − (1−p)·r_abandon·margin_rate]`。
> 同一条曲线在**下方**还有一个临界点（p=0.302 时约 $2.4）——低于它连 $0.50 的验证摩擦都不划算，最优动作退回放行。
> **这暴露了逐笔口径的一个盲区，位置很具体：低分 + 微额的合流处。**
> 注意它**不是**"金额小就一定漏"——高分段由拒绝卡住，而拒绝成本 `c_FP·(1−p)` 随 p→1 趋于 0，所以确定的欺诈总会被拒。
> 盲区只在"模型给了低分**且**金额极小"时出现；能接住它的是**跨笔的 velocity 规则**——本项目已论证、未实现（见 [`MODEL_CARD.md`](./MODEL_CARD.md) §8 第 9 条）。
> **诚实边界**：EDA 里那个「<$1 试卡欺诈簇」是**训练窗**现象；**测试窗 <$1 共 109 笔、欺诈 0 笔**，该段占测试窗 0.17%。
...
**诚实定位**：这是**离线决策系统 + 已推演工业化路径**，**不声称"生产级"**——未上线、无实时链路、无压测与灰度。

**工具说明**：设计取舍与全部技术判定由本人决定，工程实现以 Claude Code 结对完成。
仓库内**每一个数值结论都由本仓库脚本产出**，并由一致性机制保证可复算：71 个单元测试、
36 份报告的机器区哈希清单（`--verify-rerun` 对确定性档真重跑对拍）、以及 `MODEL_CARD.md` 中
每个 `[实测]` 数字的出处指针核验。**判断可以讨论，数字可以复算。**
...
6. GBDT vs GNN 的工程取舍——**对照 +0.039 PR-AUC**（过泄漏审计）；消融证增益全来自实体标签历史、结构特征≈0（已被匿名特征吸收）→ GNN 追的正是被吸收的结构；团伙 fan-out 示例（图 11）
...
> 每条的完整口径、假设、出处与已知局限见 [`MODEL_CARD.md`](./MODEL_CARD.md)。
...
版本变更见 [`MODEL_CARD.md`](./MODEL_CARD.md) §12；各步骤的原始产出（40 份分析报告）见 [`reports/`](./reports/)。

## 关键数字（记 delta，不记绝对值；跑出来再填，禁止编造）

> 评审者不关心 AUC 0.92，关心你带来多少**改善**。所有指标记成 X→Y / 差多少。
>
